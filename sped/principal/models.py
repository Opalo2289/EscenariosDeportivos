...
def nameFile(instance,filename):
    #print(siniestro.objects.last()+1)
    if EscenarioDeportivo.objects.last():
        id = EscenarioDeportivo.objects.last().id +1
    else:
        id = 1
    path =  'images/escenariodeportivo/escenariodeportivo'+str(id)+'/'+filename
    return path
...
class EscenarioDeportivo(models.Model):
    ...
    nombre = models.CharField(max_length=30)
    descripcion = models.CharField(max_length=30)
    foto1 = models.ImageField(upload_to=nameFile, blank=True, null=True)
    foto2 = models.ImageField(upload_to=nameFile, blank=True, null=True)
    def __str__(self):
        return (self.nombre) 
    class Meta:
